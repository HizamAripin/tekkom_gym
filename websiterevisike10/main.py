...
# Helper Function: Tambahkan admin default
def initialize_default_admin():
    default_admin = Admin.query.filter_by(username="defaultadmin").first()
    if not default_admin:
        hashed_password = generate_password_hash("defaultpassword", method="pbkdf2:sha256")
        new_admin = Admin(username="defaultadmin", email="admin@example.com", phone="123456789", password=hashed_password)
...
def add_membership():
    if request.method == "POST":
        ...
        if status == 'true':
            ...
            status = False  # Default to False if the status is not provided or is invalid
